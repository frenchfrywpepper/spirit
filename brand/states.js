...
//   			var formattedAddress = value.properties.formattedAddress;
	...
	function createPolygons(geometry, opts, extendBounds) {	
		...
		if (geometry.coordinates && geometry.type == 'Polygon') {
			...
			for (var i = 0; i < geometry.coordinates.length; i++){
				...
				for (var j = 0; j < geometry.coordinates[i].length; j++){
					...
					path.push(ll);
					if (extendBounds) mapBounds.extend(ll);
	...
	function readState(idx, value) {
		...
		if (abbreviation =='HI') extendBounds = false;
		for (var idx in geometryCollection) {
			createPolygons(geometryCollection[idx], opts, extendBounds);
		}
	}
	...
	$.getJSON('../data/us_states.geojson', function(json) {
